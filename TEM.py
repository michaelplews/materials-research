...
class DM3File(object):
    ...
    def plot(self, cmap="gray", *args, **kwargs):
        plt.imshow(self.imagedata, cmap=cmap, extent=[0, 2048*self.pxsize[0], 2048*self.pxsize[0], 0], norm=Normalize(self.contrastlimits[0],self.contrastlimits[1]), *args, **kwargs)
        plt.ylabel(self.pxsize[1].decode('ascii'), fontsize=10)
