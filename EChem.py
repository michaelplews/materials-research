...
class processedMPTFile(object):
	...
	def __init__(self, filename, shortname=""):
		...
		self.data_array = np.genfromtxt(filename, delimiter=None, skip_header=self.skip_line, autostrip=True, unpack=True)
		...
		if cycle:
			...
			for data_point in range (0, len(self.data_array[cycle_column])):
				if self.data_array[cycle_column][data_point] == cycle:
					xdata.append(self.data_array[x][data_point])
					ydata.append(self.data_array[y][data_point])
					#print (xdata, ydata)
				else:
					pass 
			if legend:
